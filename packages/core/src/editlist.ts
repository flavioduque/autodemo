 ...
 *
 * - `sourceMs` — time in the raw capture. Immutable: it describes what happened.
 * - `outputMs` — time in the final video. Changes whenever the edit changes.
 *
...
function segmentOutputMs(segment: EditSegment): number {
  return (segment.sourceToMs - segment.sourceFromMs) / segment.speed;
}

/** Duration of the final video: the sum of every segment's output duration. */
export function totalOutputMs(list: EditList): number {
  let total = 0;
  for (const segment of list) total += segmentOutputMs(segment);
  return total;
}

/** A segment together with where it lands in the output time base. */
type PlacedSegment = EditSegment & { outputFromMs: number; outputToMs: number };
...
function place(list: EditList): PlacedSegment[] {
  ...
  for (const segment of list) {
    const outputToMs = outputFromMs + segmentOutputMs(segment);
 ...
 */
export function outputToSource(list: EditList, outputMs: number): number | null {
  for (const segment of place(list)) {
    if (outputMs >= segment.outputFromMs && outputMs < segment.outputToMs) {
      return segment.sourceFromMs + (outputMs - segment.outputFromMs) * segment.speed;
    }
  }
  return null;
}
 ...
 */
export function sourceToOutput(list: EditList, sourceMs: number): number | null {
  for (const segment of place(list)) {
    if (sourceMs >= segment.sourceFromMs && sourceMs < segment.sourceToMs) {
      return segment.outputFromMs + (sourceMs - segment.sourceFromMs) / segment.speed;
    }
  }
  return null;
}
...
export function isCut(list: EditList, sourceMs: number): boolean {
