...
export type CursorKeyframe = { sourceMs: number; x: number; y: number; click: boolean };
...
export function cursorAt(actions: DemoAction[], sourceMs: number): CursorSample {
  ...
  if (next) {
    ...
    const approachStart = Math.max(prev.sourceMs, next.sourceMs - CURSOR_APPROACH_MS);
    if (sourceMs > approachStart) {
      const p = easeOut((sourceMs - approachStart) / (next.sourceMs - approachStart));
  ...
  for (const k of kf) {
    if (k.click && sourceMs >= k.sourceMs && sourceMs < k.sourceMs + CURSOR_PULSE_MS) {
      clickPhase = (sourceMs - k.sourceMs) / CURSOR_PULSE_MS;
      break;
    }
  }
